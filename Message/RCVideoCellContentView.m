...
#import "RCVideoCellContentView.h"
...
@implementation RCVideoCellContentView
...
- (void)drawRect:(CGRect)rect
{
    ...
    if([RCTool isIpad])
    {
        CGFloat offset_x = 25.0f;
        CGFloat offset_y = 25.0f;
        
        if(self.image)
        {
            [self.image drawInRect:CGRectMake(offset_x, offset_y, 160, 120)];
            
            UIImage* image = [UIImage imageNamed:@"play"];
            if(image)
            {
                [image drawInRect:CGRectMake(offset_x + 60,offset_y+40,40,40)];
            }
        }
        ...
        if([text length])
        {
            [text drawInRect:CGRectMake(offset_x*2 + 160, offset_y, [RCTool getScreenSize].width -(offset_x*3 + 160), 60) withFont:[UIFont systemFontOfSize:22] lineBreakMode:NSLineBreakByTruncatingTail];
        }
        ...
        if([name length])
        {
            [name drawInRect:CGRectMake(offset_x*2 + 160, offset_y, ([RCTool getScreenSize].width - (offset_x*2 + 160))/2.0, 20) withFont:[UIFont systemFontOfSize:18] lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentLeft];
        }
        ...
        if([shareCount length])
        {
            ...
            if(image)
            {
                [image drawInRect:CGRectMake(offset_x*2 + 160+ ([RCTool getScreenSize].width - (offset_x*2 + 160))/2.0, offset_y + 2, 18, 18)];
            }
            [shareCount drawInRect:CGRectMake(offset_x*2 + 160+ ([RCTool getScreenSize].width - (offset_x*2 + 160))/2.0 + 30, offset_y, ([RCTool getScreenSize].width - (offset_x*2 + 160))/2.0, 20) withFont:[UIFont systemFontOfSize:18] lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentLeft];
        }
    }
    else
    {
        CGFloat offset_x = 12.0f;
        CGFloat offset_y = 12.0f;
        
        if(self.image)
        {
            [self.image drawInRect:CGRectMake(offset_x, offset_y, 100, 75)];
            
            UIImage* image = [UIImage imageNamed:@"play"];
            if(image)
            {
                [image drawInRect:CGRectMake(offset_x + 30,offset_y+17.5,40,40)];
            }
        }
        ...
        if([text length])
        {
            [text drawInRect:CGRectMake(offset_x*2 + 100, offset_y, [RCTool getScreenSize].width -(offset_x*3 + 100), 50) withFont:[UIFont systemFontOfSize:17] lineBreakMode:NSLineBreakByTruncatingTail];
        }
        ...
        if([name length])
        {
            [name drawInRect:CGRectMake(offset_x*2 + 100, offset_y, ([RCTool getScreenSize].width - (offset_x*2 + 100))/2.0, 20) withFont:[UIFont systemFontOfSize:13] lineBreakMode:NSLineBreakByTruncatingTail alignment:NSTextAlignmentLeft];
        }
...
- (void)updateContent:(id)item delegate:(id)delegate token:(NSDictionary*)token
{
    ...
    self.image = [UIImage imageNamed:@"video_default"];
